...
export function NukeResourceReq(colony: Colony): void {
  ...
  if (nuker) {
    if (nuker.store.G < 5000 && colony.getReq(nuker.id, RESOURCE_GHODIUM) == null) {
      ...
      colony.addRequest(new resourceRequest(nuker.id, RESOURCE_GHODIUM, 4999, 5000, nuker.room));
