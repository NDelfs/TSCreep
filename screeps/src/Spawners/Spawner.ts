...
function calculateHarvesterQue(room: Room, allHarv: Creep[]): queData[]{
    let ret: queData[] = [];
    const mem: CreepMemory = { role: 'harvester', creationRoom: room.name, working: true, deliver: false, currentTarget: null, mainTarget: "", targetType: "" };
...
function spawnCreep(que: queData[], spawner: StructureSpawn): void {
    if (que.length > 0 && spawner.spawning == null) {
        ...
        if (err == OK) {
            let err = spawner.spawnCreep(que[0].body, que[0].memory.role + " " + getRandomName(), { memory: que[0].memory });
            if (err == OK) {
                console.log("spawned ", que[0].memory.role);
                que.shift();
            }
            else
                console.log("failed to spawne", que[0].memory.role, " due to ", PrettyPrintErr(err));
        }
    }
}
...
export function Spawner() {
    ...
    let harvesters = _.filter(Game.creeps, function (creep: Creep) { return creep.memory.role == "harvester" });
